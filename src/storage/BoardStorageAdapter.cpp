...
namespace Stride
{
    ...
    BoardData BoardStorageAdapter::LoadFullBoard(int boardId)
    {
        try
        {
            ...
            auto board = FromStorage(storageBoard, storageLists, allCards, allChecklist, badges, cardBadgeLinks);
            
            GL_INFO("Loaded board '{}' with {} lists and {} cards", 
                    board.title, board.lists.size(), board.GetTotalCardCount());
    ...
    BoardData BoardStorageAdapter::CreateBoard(const std::string& title, 
    ...
    CardList BoardStorageAdapter::CreateList(const std::string& boardId,
    ...
    Card BoardStorageAdapter::CreateCard(const std::string& listId,
                                         const std::string& title,
                                         const std::string& description,
                                         double position)
    {
    ...
    ChecklistItem BoardStorageAdapter::CreateChecklistItem(const std::string& cardId,
                                                           const std::string& title,
                                                           bool isCompleted,
                                                           double position)
    {
    ...
    std::string BoardStorageAdapter::CreateBadge(const std::string& cardId,
    ...
    int BoardStorageAdapter::SaveFullBoard(const BoardData& board)
    {
        try
        {
            ...
            for (size_t listIdx = 0; listIdx < board.lists.size(); ++listIdx)
            {
                ...
                double listPosition = CalculatePosition(listIdx, board.lists.size());
                ...
                for (size_t cardIdx = 0; cardIdx < list.cards.size(); ++cardIdx)
                {
                    ...
                    double cardPosition = CalculatePosition(cardIdx, list.cards.size());
                    ...
                    for (size_t itemIdx = 0; itemIdx < card.checklist.size(); ++itemIdx)
                    {
                        ...
                        double itemPosition = CalculatePosition(itemIdx, card.checklist.size());
            ...
            GL_INFO("Successfully saved board '{}' with {} lists and {} cards",
                    board.title, board.lists.size(), board.GetTotalCardCount());
    ...
    Storage::ListData BoardStorageAdapter::ToStorageList(const CardList& list, int boardId, double position)
    {
    ...
    Storage::CardData BoardStorageAdapter::ToStorageCard(const Card& card, int listId, int boardId, double position)
    {
    ...
    Storage::ChecklistItemData BoardStorageAdapter::ToStorageChecklistItem(
        const ChecklistItem& item, int cardId, double position)
    {
        ...
        const std::vector<Storage::CardBadgeData>& cardBadgeLinks)
    {
        ...
        for (const auto& storageList : sortedLists)
        {
            ...
            auto list = FromStorageList(storageList, cardsInList, storageChecklist, storageBadges, cardBadgeLinks);
    ...
    ChecklistItem BoardStorageAdapter::FromStorageChecklistItem(const Storage::ChecklistItemData& item)
    {
    ...
    double BoardStorageAdapter::CalculatePosition(size_t index, size_t total)
    {
        if (total == 0) return 1000.0;
        // Simple linear spacing: 1000, 2000, 3000, etc.
        return (index + 1) * 1000.0;
    }
