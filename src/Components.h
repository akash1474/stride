...
namespace Components
{
    ...
    inline bool ChecklistItem(const char* label, bool* checked, bool* deleted, float width)
    {
        ...
        const float textWidth = width - checkboxSize - deleteBtnSize - (innerSpacing * 2.0f) - (itemPadding * 2.0f);
        
        // Calculate height based on text
        ImVec2 textSize = ImGui::CalcTextSize(label, nullptr, false, textWidth);
        float rowHeight = (textSize.y > checkboxSize ? textSize.y : checkboxSize) + (itemPadding * 2.0f);
        ...
        if (*checked)
        {
            ImGui::RenderCheckMark(window->DrawList, ImVec2(checkRect.Min.x + 3 * dpiScale, checkRect.Min.y + 3 * dpiScale), IM_COL32(255, 255, 255, 255), checkboxSize - 6.0f * dpiScale);
        }
    ...
    inline bool InputWithIcon(const char* id, const char* icon, const char* hint, char* buffer, size_t buffer_size, float width)
    {
        const float dpiScale = FontManager::GetDpiScale();
        ...
        bool enterPressed = ImGui::InputTextWithHint(id, hint, buffer, buffer_size, ImGuiInputTextFlags_EnterReturnsTrue);
