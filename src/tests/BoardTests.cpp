...
void RegisterBoardTests(ImGuiTestEngine* engine)
{
    ...
    t->TestFunc = [](ImGuiTestContext* ctx) {
        ...
        board.mCardLists.clear();
        board.Setup(); 
        
        size_t initial_count = board.mCardLists.size();
        ...
        size_t final_count = board.mCardLists.size();
        IM_CHECK_EQ(final_count, initial_count + 1);
        
        if (final_count > 0)
        {
            IM_CHECK_STR_EQ(board.mCardLists.back().mTitle.c_str(), "Test List");
        }
    };
