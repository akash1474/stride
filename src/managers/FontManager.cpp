...
void FontManager::LoadFontsFromDirectory(const fs::path& directoryPath)
{
    ...
    for (const auto& entry : fs::directory_iterator(directoryPath))
    {
        if (entry.is_regular_file() && entry.path().extension() == ".ttf")
        {
            ...
            // We check for more specific names before more generic ones (e.g., "semibold" before "bold").
...
void FontManager::Init(float dpiScale)
{
   OpenGL::ScopedTimer timer("FontManager::Init");
    ...
    {
        fntManager.m_FontPaths[FontFamily::Regular]   = "D:/Projects/c++/stride/assets/fonts/Recursive/Recursive-Regular.ttf";
        fntManager.m_FontPaths[FontFamily::Medium]    = "D:/Projects/c++/stride/assets/fonts/Recursive/Recursive-Medium.ttf";
        fntManager.m_FontPaths[FontFamily::SemiBold]  = "D:/Projects/c++/stride/assets/fonts/Recursive/Recursive-SemiBold.ttf";
        fntManager.m_FontPaths[FontFamily::Bold]      = "D:/Projects/c++/stride/assets/fonts/Recursive/Recursive-Bold.ttf";
        fntManager.m_FontPaths[FontFamily::Black]     = "D:/Projects/c++/stride/assets/fonts/Recursive/Recursive-Black.ttf";
    }
...
void FontManager::ReloadFonts()
{
   OpenGL::ScopedTimer timer("FontManager::ReloadFonts");
    ...
    auto load_font = [&](FontFamily  /*family*/, const std::string& path) -> ImFont* {
        ...
        ImFont* font = io.Fonts->AddFontFromFileTTF(path.c_str(), fntManager.mBaseRasterFontSize, &font_config);
        if (font) {
...
ImFont* FontManager::GetFont(FontFamily family)
{
    ...
    case FontFamily::Medium:
        return fntManager.m_FontMedium;
    case FontFamily::SemiBold:
        return fntManager.m_FontSemiBold;
    case FontFamily::Bold:
        return fntManager.m_FontBold;
    case FontFamily::Black:
        return fntManager.m_FontBlack;
    default:
        return fntManager.m_FontRegular;
    }
...
void FontManager::Push(FontFamily aFamily,FontSize aSize)
{
    ...
    case FontSize::Smaller:
        size = fntManager.m_ScaledSizeSmaller;
        break;
    case FontSize::Small:
        size=fntManager.m_ScaledSizeSmall;
        break;
    case FontSize::Regular:
        size=fntManager.m_ScaledSizeRegular;
        break;
    case FontSize::Large:
        size=fntManager.m_ScaledSizeLarge;
        break;
    case FontSize::XLarge:
        size=fntManager.m_ScaledSizeXLarge;
        break;
    case FontSize::Massive:
        size=fntManager.m_ScaledSizeMassive;
        break;
    default:
        size=fntManager.m_ScaledSizeRegular;
    }
...
{
    if (str == "Regular")   return FontFamily::Regular;
    if (str == "Medium")    return FontFamily::Medium;
    if (str == "SemiBold")  return FontFamily::SemiBold;
    if (str == "Bold")      return FontFamily::Bold;
    if (str == "Black")     return FontFamily::Black;
...
void FontManager::CacheState(const fs::path& aPath)
{
    ...
    if (!file.is_open())
    {
        GL_ERROR("FontManager::CacheState - Failed to write font data to \"{}\"", aPath.generic_string());
        return;
    }
    #ifdef GL_DEBUG
...
void FontManager::ReloadStateFromCache(const fs::path& aPath)
{
    ...
    if (!ifs.is_open())
    {
        GL_INFO("FontManager::ReloadStateFromCache - No cache data found at: \"{}\"", aPath.generic_string());
        return;
    }
